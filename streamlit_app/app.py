...
Paste your Progress Code from the game's Dashboard → Export tab
to unlock deep-dive performance analytics.

Deploy at: https://streamlit.io/cloud
Repository must include this file as: streamlit_app/app.py
"""
...
# ──────────────────────────────────────────────
st.set_page_config(
    page_title="Bio-Forge Analytics | kinetic-alchemy",
    page_icon="🧬",
    layout="wide",
...
# ──────────────────────────────────────────────
with st.sidebar:
    st.markdown("## 🧬 Bio-Forge Analytics")
    st.markdown("<hr style='border-color:rgba(0,212,255,0.2)'>", unsafe_allow_html=True)
    st.markdown("**How to use:**")
    st.info(
        "1. Open the game in your browser\n"
        "2. Go to **Dashboard → Export** tab\n"
        "3. Click **Generate Progress Code**\n"
    ...
    code_input = st.text_area(
        ...
        help="Base64-encoded JSON from the game's Export tab"
    )
    load_btn = st.button("⚡ Load Analytics", use_container_width=True, type="primary")
    st.markdown("<hr style='border-color:rgba(0,212,255,0.2)'>", unsafe_allow_html=True)
    st.markdown("**Try Demo Mode**")
    demo_btn = st.button("👁 Load Demo Data", use_container_width=True)
    st.markdown("<hr style='border-color:rgba(0,212,255,0.2)'>", unsafe_allow_html=True)
    st.caption("kinetic-alchemy: The Bio-Forge © 2026")
    st.caption("Built for dietetics students & healthcare professionals")
...
DEMO_DATA = {
    ...
    "bc": 14,
    "cs": {
        "fat_soluble_vitamins":   {"correct": 8, "total": 10},
        "water_soluble_vitamins": {"correct": 9, "total": 12},
        "minerals":               {"correct": 7, "total": 8}
    },
    "ts": 1745000000000
}
...
if load_btn and code_input.strip():
    try:
        decoded = base64.b64decode(code_input.strip()).decode("utf-8")
        data = json.loads(decoded)
        st.session_state["data"] = data
    except Exception as e:
        st.error(f"❌ Invalid progress code. Please generate a fresh one from the game. ({e})")

if "data" in st.session_state and not data:
...
if not data:
    ...
    <div style="text-align:center;padding:60px 20px">
        ...
        <h1 style="font-family:'Orbitron',monospace;font-size:2rem;
            background:linear-gradient(135deg,#00d4ff,#7b2eff);
            -webkit-background-clip:text;-webkit-text-fill-color:transparent">
            THE BIO-FORGE
        </h1>
        <h2 style="font-family:'Orbitron',monospace;font-size:1rem;color:#8892b0;margin-top:8px">
            Analytics Dashboard
        </h2>
        <p style="color:#4a5268;max-width:500px;margin:24px auto;font-size:0.9rem;line-height:1.6">
            Paste your <strong style="color:#00d4ff">Progress Code</strong> from the game's 
            Dashboard → Export tab into the sidebar to unlock your performance analytics.
            Or click <strong style="color:#ffd700">Demo Mode</strong> to explore with sample data.
        </p>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
<div style="display:flex;align-items:center;gap:20px;margin-bottom:32px;flex-wrap:wrap">
    ...
    <div>
        ...
        <div style="color:#8892b0;font-size:0.8rem;margin-top:4px">Last Synced: {last_played}</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
<div style="text-align:center;color:#4a5268;font-size:0.75rem;padding:16px 0">
    ...
    Data sourced from your exported Progress Code · Grounded in Wardlaw's Contemporary Nutrition
</div>
""", unsafe_allow_html=True)
